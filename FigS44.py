...
Mono_l = ['Pdgfa', 'Vegfa', 'Vegfa', 'Cxcl12', 'Cxcl12', 'Dll4', 'Dll4']
FB_r = ['Pdgfrb', 'Flt1', 'Kdr', 'Cxcr4', 'Ackr3', 'Notch2', 'Notch4']
...
Chip_data = pd.read_csv("/jdfssz1/ST_SUPERCELLS/P21Z10200N0171/Project/SkinFullyInjury/03.RNA/02.Integrate/50.Rat_All/Immu/05.st_featureplot/chipID_Adult_510.csv")
...
for index, (lower_id, sct, upper_id) in Chip_data[['lower_ID', 'SCT', 'upper_ID']].iterrows():
    ...
    for fb_gene, mono_gene in zip(FB_r, Mono_l):
        ...
        if f"s-{ct_LR}" in adata.obsm["commot-CellChat-sum-sender"].columns and f"r-{ct_LR}" in adata.obsm["commot-CellChat-sum-receiver"].columns:
            ...
            for pt_size in [50,60,70,80,90,100]:
                ...
                plt.savefig(f"./{upper_id}_{fb_gene}_and_{mono_gene}_commot_{pt_size}.png", dpi=300)
                plt.close()
